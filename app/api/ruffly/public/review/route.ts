...
export async function GET(request: Request) {
  ...
  if (!payload || payload.typ !== "review") {
    return NextResponse.json({ error: "Invalid or expired review link." }, { status: 401 });
  }

  const destinations = destinationsForFeedbackRating(null, {
...
export async function POST(request: Request) {
  ...
  // Always return the same public destinations regardless of rating (no gating).
