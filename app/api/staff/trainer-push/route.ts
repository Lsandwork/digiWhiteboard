...
const LAST_GOOD_KEY = "trainer-push:last-good";

export async function GET() {
  try {
    const state = await getOrLoadTtlCache("trainer-push:board", BOARD_OVERLAY_CACHE_TTL_MS, () =>
      loadTrainerPushBoardState(getServiceSupabase(), { mutate: false })
    );
