...
export function SuperAdminHubPanel({
  ...
  const hubKey = hubTab as (typeof SUPER_ADMIN_HUB_TABS)[number];
  const hub = SUPER_ADMIN_HUBS[hubKey];
...
export function SuperAdminNestedReturnBar({
  ...
  return (
    <div className="mb-4 flex flex-wrap items-center gap-2 rounded-xl border border-admin-border bg-black/20 px-3 py-2">
      ...
      <span className="text-xs text-admin-muted">Opened from the Super Admin menu hubs — full tools still work the same.</span>
    </div>
  );
}
  ...
  return (
    <section className="space-y-5">
      <header className="rounded-2xl border border-admin-border bg-gradient-to-br from-[#132033] via-[#101826] to-[#0b1220] p-5">
        <p className="text-xs uppercase tracking-[0.18em] text-admin-muted">Super Admin</p>
      ...
      <span className="flex items-start gap-3">
        ...
        <span className="min-w-0">
          <span className="flex items-center gap-1.5 text-sm font-medium text-white">
            {label}
            {link.kind === "route" ? <ExternalLink className="h-3.5 w-3.5 text-admin-muted" /> : <ArrowUpRight className="h-3.5 w-3.5 text-admin-muted" />}
          </span>
