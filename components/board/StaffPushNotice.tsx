...
import { PushNoticeFlashLayers } from "@/components/board/PushNoticeFlashLayers";
import {
...
function NoticeContent({ notice, fullscreen = false }: { notice: StaffPushNotice; fullscreen?: boolean }) {
  ...
  const isAlerting = useStaffPushNoticeAlarm(notice);
  const isDogHandler = isDogHandlerComplaintNotice(notice);
  const isImportant = isDailyReminder && notice.priority === "important";

  return (
    <article
      className={`staff-push-notice ${isDailyReminder ? "staff-push-notice--daily-reminder" : "staff-push-notice--alert"} ${isImportant ? "staff-push-notice--daily-reminder-important" : ""} ${isDogHandler ? "staff-push-notice--dog-handler" : ""} ${isAlerting ? "staff-push-notice--burst" : ""} ${fullscreen ? "staff-push-notice--fullscreen" : ""}`}
      role="alert"
      aria-live="assertive"
    >
...
export function StaffPushNoticeTvOverlay({ active, notice }: { active: boolean; notice?: StaffPushNotice | null }) {
  ...
  return (
    <>
      ...
      <div className={`staff-push-notice-tv-flash staff-push-notice-tv-flash--secondary staff-push-notice-tv-flash--${tone}`} aria-hidden="true" />
...
export function StaffPushNoticeFullscreen({ notice }: { notice: StaffPushNotice }) {
  ...
  return (
    <section
      className={`staff-push-notice-fullscreen ${isDailyReminder ? "staff-push-notice-fullscreen--daily-reminder staff-push-notice-fullscreen--alert" : "staff-push-notice-fullscreen--alert"}`}
      aria-label={isDailyReminder ? "Active daily reminder" : "Active yard handler alert"}
    >
