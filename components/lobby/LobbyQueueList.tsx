    ...
    <section className="lobby-queue">
      ...
      <div className="space-y-3">
        {dogs.map((dog) => (
          <article
            key={dog.id}
            className="lobby-queue-row relative overflow-hidden rounded-2xl border border-white/10 bg-ink-900/55 px-4 py-3 sm:px-5 sm:py-4"
          >
            <Image src={lobbyAssets.queueRowBg} alt="" fill className="pointer-events-none object-cover opacity-40" />
            <div className="relative z-10 flex items-center gap-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                {dog.breed ? <p className="truncate text-base text-slate-400 sm:text-lg">{dog.breed}</p> : null}
              </div>
              <p className="shrink-0 rounded-full bg-fitdog-orange/15 px-4 py-2 text-sm font-semibold text-orange-100 sm:text-base">
                {dog.checkout_status}
              </p>
            </div>
          </article>
