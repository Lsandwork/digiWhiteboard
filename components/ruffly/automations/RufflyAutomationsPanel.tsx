...
export function RufflyAutomationsPanel({ enabled = true }: { enabled?: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch("/api/ruffly/automations", { cache: "no-store" });
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h2 className="text-2xl font-semibold tracking-tight text-[#1f2933]">Automations</h2>
          <p className="mt-1 text-sm text-slate-500">Event-driven workflows. Templates start disabled.</p>
        </div>
