...
export function RufflyPublicReviewForm({ token }: { token: string }) {
  ...
  useEffect(() => {
    const timer = window.setTimeout(async () => {
      const res = await fetch(`/api/ruffly/public/review?token=${encodeURIComponent(token)}`);
  ...
  async function submit() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/ruffly/public/review", {
        method: "POST",
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(160deg,#fff8f3,#fff)] px-4 py-12 text-[#1f2933]">
      <div className="mx-auto max-w-lg rounded-3xl border border-orange-100 bg-white p-6 shadow-sm">
        ...
        <h1 className="text-2xl font-semibold">How was your Fitdog visit?</h1>
        <p className="mt-2 text-sm text-slate-600">
          Your feedback helps our team. Public review options are always available — we never hide them based on your
          rating.
        </p>
        ...
        {!submitted ? (
          ...
          <div className="mt-6 space-y-3">
            <p className="text-sm text-slate-700">Thank you — your feedback was received.</p>
            {destinations.length ? (
              <div className="space-y-2">
                <p className="text-sm font-medium">Share a public review if you’d like:</p>
              ...
              <p className="text-sm text-slate-500">Public review destinations will appear once configured by Fitdog.</p>
            )}
          </div>
        )}
      </div>
