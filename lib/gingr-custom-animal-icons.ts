...
export async function fetchCurrentlyCheckedInDogs(options?: {
  ...
  const { fetchCurrentlyCheckedInDogsRobust } = await import("@/lib/gingr-checked-in-dogs");
  const result = await fetchCurrentlyCheckedInDogsRobust({ force: options?.force });
  checkedInCache = { dogs: result.dogs, cachedAt: Date.now() };
  return result.dogs;
}
