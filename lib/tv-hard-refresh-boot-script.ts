 ...
 * Each poll aborts after 3s so a hung nonce endpoint cannot stick `busy` forever.
 */
export const TV_HARD_REFRESH_BOOT_SCRIPT = `(function(){try{var path=location.pathname||"/";if(path.indexOf("/admin")===0||path.indexOf("/blog")===0||path.indexOf("/api")===0||path.indexOf("/ruffly")===0)return;var board=path==="/"||path.indexOf("/lobby")===0||path.indexOf("/cast")===0||path.indexOf("/staff-cast")===0||path.indexOf("/lobby-cast")===0||path.indexOf("/display")===0||path.indexOf("/boards")===0;if(!board)return;var q=${JSON.stringify(TV_HARD_REFRESH_QUERY)};try{var clean=new URL(location.href);if(clean.searchParams.has(q)){clean.searchParams.delete(q);history.replaceState(null,"",clean.pathname+clean.search+clean.hash);}}catch(e0){}var seen=null;var busy=false;function visit(){try{var url=new URL(location.href);url.searchParams.delete(q);url.searchParams.set(q,String(Date.now()));location.replace(url.toString());return;}catch(e1){}try{location.assign(location.pathname+location.search+location.hash);return;}catch(e2){}try{location.reload();}catch(e3){}}function poll(){if(busy)return;busy=true;var ctrl=null;var timer=null;try{if(typeof AbortController!=="undefined"){ctrl=new AbortController();timer=setTimeout(function(){try{ctrl.abort()}catch(e4){}},3000);}}catch(e5){}fetch(${JSON.stringify(TV_HARD_REFRESH_ENDPOINT)},{cache:"no-store",credentials:"same-origin",signal:ctrl?ctrl.signal:undefined}).then(function(res){return res.ok?res.json():null;}).then(function(body){if(!body)return;var nonce=Number(body.nonce);if(!isFinite(nonce))return;if(seen===null){seen=nonce;return;}if(nonce!==seen)visit();}).catch(function(){}).then(function(){if(timer)clearTimeout(timer);busy=false;});}poll();setInterval(poll,${TV_HARD_REFRESH_POLL_MS});document.addEventListener("visibilitychange",function(){if(document.visibilityState==="visible")poll();});}catch(e){}})();`;
