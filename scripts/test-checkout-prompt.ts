...
assert.equal(isPromptedCheckoutRecord({ checkout_prompted: true }), true);
assert.equal(
...
assert.equal(isPromptedCheckoutRecord({ gingr_event_type: "send_to_front" }), true);
assert.equal(isPromptedCheckoutRecord({ source: "coordinator_prompt" }), true);
...
assert.equal(
  isPromptedCheckoutDog({
    ...checkoutDog({ source: "gingr_webhook", webhook_type: "checking_out" }),
