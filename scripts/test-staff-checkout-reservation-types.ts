...
assert.equal(
  mapped.some((dog) => dog.reservation_type === "Training & Classes" && dog.display_status === "checking_out"),
  true,
  "Training & Classes checkouts must map onto the board"
);
assert.equal(
  mapped.some((dog) => dog.animal_name === "ClassPup" && dog.display_status === "checking_in"),
  true,
  "Training & Classes checking-in-soon rows must appear"
);
...
const merged = mergeBackOfHouseRecords(
  [{ id: 1, a_first: "Near", type: "Hourly" }],
  [
    { id: 1, a_first: "Near", type: "Hourly" },
    { id: 26, a_first: "Class", type: "Training & Classes", type_id: 26 }
  ]
);
