-- Soft-archive duplicate same-day trainer commission entries, then guard new inserts.
-- Duplicate key: trainer + client + dog + package + sale_date + final_commission_cents
  ...
  select
    ...
    row_number() over (
      ...
      order by
        ...
        case when source = 'csv_import' then 1 else 0 end,
...
set
  ...
  internal_notes = trim(
    both E'\n' from concat_ws(
      ...
      'Archived as duplicate same-day entry (migration 050).'
    )
  ),
  updated_at = now()
from ranked
...
create unique index if not exists package_commission_records_same_day_dedupe_idx
...
comment on index public.package_commission_records_same_day_dedupe_idx is
  'Prevents duplicate active trainer commission rows for the same sale day entry.';
